-- Fix missing start_date/last_reward_date/end_date on investments
...
SET
  start_date = COALESCE(i.start_date, i.created_at, now()),
  last_reward_date = COALESCE(i.last_reward_date, COALESCE(i.start_date, i.created_at, now())),
  end_date = COALESCE(
    i.end_date,
    COALESCE(i.start_date, i.created_at, now()) + (
      COALESCE(it.cycle_days, 1) * COALESCE(it.total_cycles, it.duration) || ' days'
    )::interval
  )
